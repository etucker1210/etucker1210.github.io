function Output= Calc_Lya(data,footfall,timelag)
    ...
    if ~isempty(data(i).res) 
        for j=1:8
            if sum(~isnan(FD{i}(j,:)))>=2
                ...
                % filter data
%                 [xfilt,b,a]=lopass_butterworth(Xnew,40,500,4);
%                 [yfilt,b,a]=lopass_butterworth(Ynew,40,500,4);
                ...
                SSlegx{i}{j}(:,1) = sxnorm-nanmean(sxnorm);
                SSlegy{i}{j}(:,1) = synorm-nanmean(synorm);
                [theta,rho]= cart2pol(sxnorm-nanmean(sxnorm),synorm-nanmean(synorm));
%                 theta = theta-nanmean(theta);
                SSlegtheta{i}{j}(:,1) = theta-nanmean(theta) ;
                SSlegrho{i}{j}(:,1) = rho-nanmean(rho);
                clear index a b xfilt yfilt temp* *nan* posx posy... 
                theta theta series* *new samples numstrides t sx* sy* rho
            else
                poslegX{i}{j} = [];
                poslegY{i}{j} = [];
                SSlegx{i}{j}= [];
              ...
              for j = 1:8
                  for i = 1:length(loc)
                      if ~isempty(SSlegtheta{loc(i)})
                        ...
                        serieslegrho{j} = vertcat(serieslegrho{j},SSlegrho{loc(i)}{j});
                  ...
                  [rhofilt,b,a]= lopass_butterworth(serieslegrho{j},30,500,4);
                  clear a b 
%                  % if filtering second AND taking mean of the whole
%                  thing.
    %                   serieslegtheta{j} = thetafilt-nanmean(thetafilt);
    %                   serieslegx{j} = xfilt-nanmean(xfilt);
    %                   serieslegy{j} = yfilt-nanmean(yfilt);
    
                   % if filtering second and not taking mean 
                      serieslegtheta{j} = thetafilt;
                      serieslegx{j} = xfilt;
                      serieslegy{j} = yfilt;
                      serieslegrho{j} = rhofilt;
%                   %if not filtering second
%                   serieslegtheta{j} = serieslegtheta{j}-nanmean(serieslegtheta{j});
%                   serieslegx{j} = serieslegx{j}-nanmean(serieslegx{j});
%                   serieslegy{j} = serieslegy{j}-nanmean(serieslegy{j});
%                   
              end
              if ~isempty(SSlegtheta{loc(i)})
                  sFrq(9) = nanmean(sFrq);
                  ...
                  Output(count).serieslegrho = serieslegrho;
              ...
              clear state sF duty* df sF* slen sL* rho*
          end
          clear loc
      end
  end
  clear i j count names treatments theta* t yfilt xfilt series* pos* spider*
...
%             dimmlegtheta(j) = max(find(FNN~=0))+1;
%             clear FNN
%             [FNN]=fnn_deneme(Output(i).serieslegrho{j},10,10,15,2);
%             dimmlegrho(j) = max(find(FNN~=0))+1;
%             clear FNN
%             [FNN]=fnn_deneme(Output(i).serieslegx{j},10,10,15,2);
%             dimmlegx(j) = max(find(FNN~=0))+1;
%             clear FNN
%             [FNN]=fnn_deneme(Output(i).serieslegy{j},10,10,15,2);
%             dimmlegy(j) = max(find(FNN~=0))+1;
%             clear FNN
...
%         end
%     end
%     Output(i).dimmlegtheta = dimmlegtheta;
%     Output(i).dimmlegrho = dimmlegrho;
%     Output(i).dimmlegx = dimmlegx;
%     Output(i).dimmlegy = dimmlegy;
%     Output(i).avgdimtheta = nanmean(dimmlegtheta);
%     Output(i).avgdimrho = nanmean(dimmlegrho);
%     Output(i).avgdimx = nanmean(dimmlegx);
%     Output(i).avgdimy = nanmean(dimmlegy);
%     
%     clear dimm*
% end
% 
  ...
  for i = 1:size(Output,2)
    for j = 1:8
        ...
        stateRholeg{j}(:,d)=Output(i).serieslegrho{j}(1+(d-1)*timelag:end-(embeddimleg-d)*10);
        end
    end
    Output(i).stateThetaleg = stateThetaleg;
    Output(i).stateRholeg= stateRholeg
  ...
  for i = 1:size(Output,2)
      for j = 1:8
          ...
          Lyalegrho{j} = ldslegtheta;
          [divergence,ldslegrho]=lds_calc(Output(i).stateRholeg{j},ws,fs,period, plotje);
          Lyalegtheta{j} = ldslegrho;
      ...
      Output(i).LyapunovLegRho =Lyalegrho;
